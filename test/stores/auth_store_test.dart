...
void main() {
  ...
  group('AuthStore class', () {
    group('login action', () {
      test(
          'When the login action is called, the user is authenticated successfully',
          () async {
        final user = User(
          id: 1,
          name: 'Test User',
      ...
      test(
          'When the login action is called, the user is not authenticated successfully',
          () async {
        when(mockAuthService.login(any, any))
            .thenThrow(HttpException(HttpStatus.unauthorized, "Unauthorized"));
        ...
        verify(mockSnackBarService.show(any, duration: anyNamed('duration')))
            .called(1);
      });
    });
    ...
    group('logout action', () {
      test(
          'When the logout action is called, the user is logged out successfully',
          () async {
        final authStore = AuthStore();
      ...
      test(
          'When the logout action is called, the user is not logged out successfully',
          () async {
        final authStore = AuthStore();
